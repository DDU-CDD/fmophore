    ...
    for file_name in files_to_copy:
        ...
        if file_name in ["DFTB", "MP2"]:  
            ...
            updated_content = content.replace(
                "/FMOPhore/QM_run/", f"{package_path}/QM_run/")
            with open(destination, "w") as f:
                f.write(updated_content)

def execute_run_MP2():
    for input_file in [f for f in os.listdir() if f.endswith(".inp")]:
        execute_MP2(input_file)
def execute_run_DFTB():
    for input_file in [f for f in os.listdir() if f.endswith(".inp")]:
        execute_DFTB(input_file)

if args.qm_calculation == "DFTB":
    files_to_copy = ["rungms", "DFTB"]
elif args.qm_calculation == "MP2": 
    files_to_copy = ["rungms", "MP2"]
####################################################################################################
